...
class CiscoASALogGenerator(AIEntityMixin):
    ...
    def _internal_ip(self, side):
        """An internal address for one end of the session — see the src/dest split."""
        return self._entity_field(
            'internal_ips', lambda: self._ip(self.internal_ips), side=side)
    ...
    def _gen_vpn(self):
        user  = self._ai_user('users', self.users)
    ...
    def _gen_auth(self):
        user = self._ai_user('users', self.users)
        ip   = self._internal_ip('src')
        srv  = self._internal_ip('dest')
    ...
    def _gen_acl(self):
        ...
        events = [
            ...
            # [cisco_intrusion] matches on these message ids and is what grants
            # attack+ids, so without them the add-on's Intrusion_Detection path
            # can never fire however the traffic looks.
            (2, '106016', f'Deny IP spoof from ({src_ip}) to {dst_ip} on interface {iface}'),
            (2, '106017', f'Deny IP due to Land Attack from {src_ip} to {src_ip}'),
            (4, '400032', f'IDS:2151 Large ICMP packet from {src_ip} to {dst_ip} on interface {iface}'),
        ]
        sev, msgid, msg = random.choice(events)
        return self._line(sev, msgid, msg)
