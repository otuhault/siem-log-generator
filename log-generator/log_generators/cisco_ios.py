...
class CiscoIOSLogGenerator(AIEntityMixin):
    ...
    def generate(self):
        ...
        # One message describes one host: pick it before the address and MAC
        # below, so they belong to the same A&I entity.
        self._new_entity()
    ...
    def _random_internal_ip(self):
        """The current entity's address, else one from the pool."""
        return self._entity_field('internal_ips',
                                  lambda: random.choice(self.internal_ips))
    ...
    def _pick_mac(self):
        """MAC in Cisco dot notation, from the current entity when it has one."""
        raw = self._entity_field('mac_addresses', self._random_mac)
    ...
    def _generate_event(self, event_type):
        ...
        # --- SYSTEM ---
            ...
            # Two spaces after the user name: [EXTRACT-cisco_ios-cfglog_loggedcmd]
            # reads `User:(\S+)\s\slogged command:(.+)`, so with one space the
            # add-on extracts neither the user, the action nor the command.
        ...
        # --- HARDWARE ---
            ...
            server = random.choice(self.ntp_servers)
            # Severity 4 is what cisco_ios_messages.csv carries for this
            # mnemonic; at 6 the lookup misses and the enrichment is lost.
            return self._format_log('NTP', 4, 'PEERUNREACH', f'Peer {server} is unreachable')
