...
class PaloAltoLogGenerator(AIEntityMixin):
    ...
    METADATA = {
        ...
        'sources': [
            {'id': 'traffic', 'name': 'Traffic', 'sourcetype': 'pan:traffic', 'description': 'Network traffic logs (sessions, bytes, packets, actions)'},
            {'id': 'threat',  'name': 'Threat',  'sourcetype': 'pan:threat',  'description': 'Security threat logs (vulnerabilities, malware, spyware, URLs)'},
            {'id': 'system',  'name': 'System',  'sourcetype': 'pan:system',  'description': 'System events (authentication, configuration, HA, upgrades)'},
        ],
    }
    ...
    def __init__(self, log_types=None):
        ...
        # Phase 4c overrides (set by environment_manager.inject_network_pools_into)
        # When non-empty, these pools take precedence over the outbound/inbound logic
        # for the src_ip and dest_ip CIM fields respectively.
        self.src_ip_pool: list = []
        self.dest_ip_pool: list = []

    def generate(self):
        """Generate a single Palo Alto log line"""
        # One event describes one machine and, when A&I pins one, its
        # account: pick the entity before building the body.
        self._new_entity()
    ...
    def _internal_ip(self, side):
        """An internal address for one end of the session.

        `internal_ips` feeds both: the client on the way out, the server on the
        way in. Drawing them from one flat pool made a session look like two
        unrelated machines, so each end names the side it means and reads the
        entity A&I put there.
        """
        return self._entity_field(
            'internal_ips', lambda: self._get_random_ip(self.internal_ips), side=side)
    ...
    def _generate_traffic_log(self):
        ...
        if is_outbound:
            ...
            dst_ip = self._internal_ip('dest')
            src_zone = random.choice(self.zones['external'])
            dst_zone = random.choice(self.zones['internal'])

        # Phase 4c: per-direction pool overrides (CIM-aligned: src_ip/dest_ip
        # always pick from src_ip_pool/dest_ip_pool when set, regardless of direction)
        if self.src_ip_pool:
            src_ip = random.choice(self.src_ip_pool)
        if self.dest_ip_pool:
            dst_ip = random.choice(self.dest_ip_pool)

        # A firewall's traffic log is overwhelmingly session teardowns: `end` is
        # written for every session, `start` only when the rule enables it, and
        # deny/drop only for what was blocked. Drawing the four uniformly made
        # nearly half the traffic a block, and — since Splunk_TA_paloalto 4.0.0
        # fixed the operator precedence that let every subtype match
        # [pan_traffic_start] — those events now reach no datamodel at all.
        subtype = random.choices(self.traffic_subtypes,
                                 weights=[20, 60, 10, 10], k=1)[0]
        ...
        if is_outbound and self.usernames and random.random() < 0.7:
            src_user = self._ai_user('usernames', self.usernames)
    ...
    def _generate_threat_log(self):
        ...
        # Phase 4c: per-direction pool overrides
        if self.src_ip_pool:
            src_ip = random.choice(self.src_ip_pool)
        if self.dest_ip_pool:
            dst_ip = random.choice(self.dest_ip_pool)
