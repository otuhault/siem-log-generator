...
from .windows_xml import render as render_windows_xml
import xml.etree.ElementTree as ET
from xml.dom import minidom

class WindowsEventLogGenerator(AIEntityMixin):
    ...
    METADATA = {
        ...
        'sources': [
            {'id': 'Security',    'name': 'Security',    'sourcetype': 'WinEventLog:Security',    'description': 'Authentication, privileges, processes (4624, 4625, 4688, etc.)'},
            {'id': 'Application', 'name': 'Application', 'sourcetype': 'WinEventLog:Application', 'description': 'Application errors, installations, updates (1000, 1001, 11707, etc.)'},
            {'id': 'System',      'name': 'System',      'sourcetype': 'WinEventLog:System',      'description': 'Service management, system events (6005, 7036, 7034, etc.)'},
        ],
    }
    ...
    def _setup_event_types(self):
        ...
        # Filled by EnvironmentManager.inject_into() from A&I entity IPs.
        # Empty means "no environment": fall back to a random RFC1918 address.
        self.ip_addresses = []
    ...
    def generate(self):
        ...
        # One event describes one machine: pick it before building the body so
        # the hostname and the address below belong to the same entity.
        self._new_entity()
        ...
        # The templates below are indented for readability; render() puts the
        # event in the one-line, single-quoted form Windows ships and the add-on
        # extracts from. See windows_xml.py.
        if self.source == 'Security':
            event_xml = self._generate_security_event(event_id, description)
        elif self.source == 'Application':
            event_xml = self._generate_application_event(event_id, description)
        elif self.source == 'System':
            event_xml = self._generate_system_event(event_id, description)
        else:
            event_xml = self._generate_generic_event(event_id, description)
  ...
  <EventData>
    ...
    def _generate_classic_format(self, event_id, description):
        ...
        username = self._ai_user('usernames', self.usernames)
...
\tLogon ID:\t\t0x{random.randint(100000, 999999):x}'''
  ...
  <EventData>
    ...
    <Data Name="TargetUserSid">{self._sid_for(username, domain)}</Data>
  ...
  <EventData>
    <Data Name="SubjectUserSid">{self._sid_for(username, domain)}</Data>
    <Data Name="SubjectUserName">{username}</Data>
    <Data Name="SubjectDomainName">{domain}</Data>
    <Data Name="SubjectLogonId">0x{random.randint(100000, 999999):x}</Data>
  ...
  <EventData>
    ...
    def _get_source_ip(self):
        """The current entity's address, else a random one."""
        return self._entity_field(
            'ip_addresses',
            lambda: (random.choice(self.ip_addresses) if self.ip_addresses
                     else f'192.168.{random.randint(1, 255)}.{random.randint(1, 254)}'))

    def _sid_for(self, account, domain=None):
        """A stable SID, so TargetUserSid tracks TargetUserName.

        A real SID is S-1-5-21-<three domain sub-authorities>-<RID>: the domain
        part is identical for every account of a domain, the RID identifies the
        account. Drawing all four at random turned every event into a new
        identity, which is what CIM's user_id is meant to prevent.
        """
        dom = domain or (self.domains[0] if self.domains else 'WORKGROUP')
        digest = hashlib.sha1(dom.encode('utf-8')).hexdigest()
        parts = '-'.join(str(int(digest[i:i + 8], 16)) for i in (0, 8, 16))
        rid = 1000 + int(
            hashlib.sha1(f'{dom}\\{account}'.encode('utf-8')).hexdigest()[:8], 16
        ) % 9000
        return f'S-1-5-21-{parts}-{rid}'

    def _get_random_workstation(self):
        """The current entity's hostname, else a generated one."""
        return self._entity_field('workstations', self._random_workstation)

    def _random_workstation(self):
