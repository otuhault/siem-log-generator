...
class ZscalerLogGenerator(AIEntityMixin):
    ...
    METADATA = {
        ...
        'sources': [
            {'id': 'web',    'name': 'Web Proxy', 'sourcetype': 'zscalernss-web',
             'description': 'NSS web proxy transactions (URL categories, threats, DLP, users)'},
            {'id': 'tunnel', 'name': 'Tunnel',    'sourcetype': 'zscalernss-tunnel',
    ...
    def generate(self):
        # One record describes one device: its name, address and OS must agree.
        self._new_entity()
    ...
    def _generate_web(self):
        ...
        client_ip    = self._entity_field(
            '_INTERNAL_IPS', lambda: self._rand_ip(self._INTERNAL_IPS))
        ...
        # Weighted, not uniform: a proxy sees overwhelmingly clean traffic, and a
        # generator where 5 events in 6 carry a threat makes every detection fire.
        threat_cat   = random.choices(
            self._threat_cats, weights=[88, 3, 4, 3, 1, 1], k=1)[0]
        ...
        # Same reasoning as the threat category: a DLP engine fires on a small
        # share of transactions, not four in five.
        dlp_engine   = random.choices(
            self._dlp_engines, weights=[93, 2, 2, 2, 1], k=1)[0]
        dlp_dicts    = 'None' if dlp_engine == 'None' else random.choice(self._dlp_dicts[1:])
        rule_type    = ('DLP' if dlp_engine != 'None'
                        else 'Malware' if threat_cat != 'None'
                        else 'URL Filtering')
        ...
        fields = [
            ...
            # [Zscaler_Proxy_DLP] matches ruletype="DLP"; the field decides which
            # policy produced the verdict, so it follows what the event carries.
            ...
            f"devicename={self._entity_field('_device_names', lambda: random.choice(self._device_names))}",
            f"deviceostype={self._entity_field('_device_os_types', lambda: random.choice(self._device_os_types))}",
