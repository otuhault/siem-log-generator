...
// Assets & Identities used to be a top-level tab, and this module loaded its
// data when that tab was clicked. It is now a sub-tab of Configuration, the
// listener matched nothing, and the lists stayed empty until something was
// created. app.js routes every Configuration sub-tab to its loader, so this one
// is exposed for it — on window, because this module is loaded by its own
// <script> tag and importing it again under another URL would bind every
// handler twice.
...
async function loadEnvData() {
    try {
        ...
        renderEnvSettings(meta.settings || {});
...
/**
 * The AD domain is a property of the environment, not of any one entity.
 * Only the Windows and Active Directory generators consume it; the others have
 * no notion of a domain, which is why it is not a column on entity/account.
 */
function renderEnvSettings(settings) {
    const nt = document.getElementById('adDomain');
    const dns = document.getElementById('adDomainDns');
    if (nt) nt.value = settings.ad_domain || '';
    if (dns) dns.value = settings.ad_domain_dns || '';
}

function setupEnvSettings() {
    const btn = document.getElementById('saveEnvSettingsBtn');
    if (!btn) return;
    btn.addEventListener('click', async () => {
        const payload = {
            ad_domain:     document.getElementById('adDomain').value.trim(),
            ad_domain_dns: document.getElementById('adDomainDns').value.trim(),
        };
        try {
            ...
            window.showNotification?.('Environment settings saved', 'success');
        } catch (err) {
            console.error('Failed to save environment settings:', err);
            window.showNotification?.('Failed to save environment settings', 'error');
        }
    });
}
...
function setupAddButtons() {
    document.getElementById('addEntityBtn').addEventListener('click', () => {
        ...
        document.getElementById('entityFormTitle').textContent = 'Add Asset';
        document.getElementById('submitEntityBtn').textContent = 'Add Asset';
    ...
    document.getElementById('addAccountBtn').addEventListener('click', () => {
        ...
        document.getElementById('accountFormTitle').textContent = 'Add Identity';
        document.getElementById('submitAccountBtn').textContent = 'Add Identity';
...
window.editEntity = function(id) {
    ...
    document.getElementById('entityFormTitle').textContent  = 'Edit Asset';
    document.getElementById('submitEntityBtn').textContent  = 'Update Asset';
...
window.editAccount = function(id) {
    ...
    document.getElementById('accountFormTitle').textContent  = 'Edit Identity';
    document.getElementById('submitAccountBtn').textContent  = 'Update Identity';
