...
export function validateSenderForm(form) {
    ...
    if (destinationType === 'file') {
        ...
        // Phase 4b: pick from persisted Syslog Destinations registry
        const sysField = form.querySelector('#syslogDestinationSelect');
        if (sysField && !validateField(sysField, [{ type: 'required' }])) {
            isValid = false;
            errors.push('Please select a syslog destination');
        }
    }
...
export function validateConfigurationForm(form) {
    ...
    // Optional fields with validation if present in the DOM (kept for forward-compat)
